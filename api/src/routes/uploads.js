...
const AUTO_INGEST_DEFAULT = (() => {
  const parsed = parseBoolean(process.env.PIPELINE_AUTO_INGEST);
...
uploadsRouter.post(
  ...
  async (req, res) => {
    try {
      ...
      const rawAutoIngest = req.body?.autoIngest;
      const parsedAutoIngest = parseBoolean(rawAutoIngest);
      const autoIngestRequested = parsedAutoIngest === undefined ? AUTO_INGEST_DEFAULT : parsedAutoIngest;

      let ingestOptions = {};
      const ingestOptionsField = req.body?.ingestOptions;
      if (typeof ingestOptionsField === 'string' && ingestOptionsField.trim()) {
        try {
          ingestOptions = JSON.parse(ingestOptionsField);
        } catch (err) {
          console.warn('Failed to parse ingestOptions JSON:', err);
        }
      }
      ...
      for (const key of passthroughKeys) {
        if (Object.prototype.hasOwnProperty.call(req.body || {}, key) && ingestOptions[key] === undefined) {
          ingestOptions[key] = req.body[key];
        }
      }

      const sanitizedIngestOptions = sanitizeIngestionOptions(ingestOptions);
      ...
      let autoIngestError = null;

      if (autoIngestRequested) {
        try {
          const uploadRecord = await fetchUploadWithOwner(uploadId, req.user.id);
          if (uploadRecord) {
            const job = await queueIngestionJob({
              upload: uploadRecord,
              user: req.user,
              options: sanitizedIngestOptions,
            });
            ingestionJobId = job.jobId;
            autoIngestStarted = true;
            inserted.status = 'processing';
          } else {
            autoIngestError = new Error('upload not found after creation');
          }
        } catch (err) {
          autoIngestError = err;
          console.error('Auto ingestion queue failed:', err);
        }
      }
      ...
      res.status(201).json({
        ...
        autoIngestStarted,
        ingestionJobId,
        ingestionError: autoIngestError ? autoIngestError.message : null,
      });
    } catch (err) {
